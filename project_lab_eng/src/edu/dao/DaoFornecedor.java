...
public class DaoFornecedor implements IDaoFornecedor {
	...
	public void adicionar(Fornecedor f) {
		try {
			...
			sb.append("fone1, fone2, status, dt_cadastro, cnpj_fornecedor, contato, serv_prod, observacao ) ");
			sb.append("VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			st.setString(15, f.getServProd());
			st.setString(16, f.getObservacao());
	...
	public void alterar(Fornecedor f) {
		try {
			...
			sb.append("fone1 = ?, fone2 = ?, status = ?, contato = ?, serv_prod = ?, observacao = ? ");
			...
			st.setString(13, f.getServProd());
			st.setString(14, f.getObservacao());
			st.setString(15, f.getCnpj());
			st.executeUpdate();
			con.close();
		} catch (ClassNotFoundException | SQLException e) {
			e.printStackTrace();
	...
	public List<Fornecedor> listar() {
		...
		try {
			...
			while (rs.next()) {
				...
				f.setServProd(rs.getString("serv_prod"));
